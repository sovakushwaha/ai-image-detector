"""Train EfficientNet-B0 transfer learning (Stage 19).

Experiment ID: efficientnet_b0_transfer_v1

PART A — Preflight verification (one batch forward, no backward)
...
EXPERIMENT_ID = "efficientnet_b0_transfer_v1"
RANDOM_SEED = 42
BATCH_SIZE = 32
NUM_WORKERS = 0
EXPECTED_TRAIN = 1376
...
PHASE1_CKPT = MODELS_DIR / "efficientnet_b0_phase1_best_v1.pt"
PHASE2_CKPT = MODELS_DIR / "efficientnet_b0_phase2_best_v1.pt"
SELECTED_CKPT = MODELS_DIR / "efficientnet_b0_selected_v1.pt"
PHASE1_HISTORY = RESULTS_DIR / "efficientnet_b0_phase1_history_v1.csv"
PHASE2_HISTORY = RESULTS_DIR / "efficientnet_b0_phase2_history_v1.csv"
REPORT_PATH = RESULTS_DIR / "efficientnet_b0_training_report_v1.txt"
LOSS_FIG = FIGURES_DIR / "efficientnet_b0_transfer_loss_v1.png"
AUC_FIG = FIGURES_DIR / "efficientnet_b0_transfer_auc_v1.png"
AP_FIG = FIGURES_DIR / "efficientnet_b0_transfer_ap_v1.png"
...
def plot_curves(phase1_df: pd.DataFrame, phase2_df: pd.DataFrame) -> None:
    ...
    _style(ax, "Loss", "EfficientNet-B0 transfer: train/val loss")
    fig.tight_layout()
    ...
    _style(ax, "ROC-AUC", "EfficientNet-B0 transfer: validation ROC-AUC")
    fig.tight_layout()
    ...
    _style(ax, "Average Precision", "EfficientNet-B0 transfer: validation AP")
    fig.tight_layout()
...
def main() -> None:
    ...
    print("=" * 60)
    print("Stage 19 — EfficientNet-B0 transfer development")
    print("=" * 60)
    ...
    # ============================================================
    print("\n" + "=" * 60)
    print("PART A — Preflight verification (no backward)")
    print("=" * 60)

    preflight_model = EfficientNetB0BinaryV1(weights=DEFAULT_WEIGHTS)
    preflight_total, preflight_trainable = count_parameters(preflight_model)
    preflight_head = count_binary_head_parameters(preflight_model)
    print(f"Weights enum: {preflight_model.weights_name}")
    print(f"Weights source: {preflight_model.weights_url}")
    print(f"Original classifier output size: {preflight_model.original_classifier_output_size}")
    print(f"Replacement classifier output size: {preflight_model.binary_classifier_output_size}")
    print(f"Total binary-model parameters: {preflight_total}")
    print(f"Trainable parameters (preflight): {preflight_trainable}")
    print(f"Replacement binary head parameters: {preflight_head}")

    preflight_model = preflight_model.to(device)
    images, labels, _ = next(iter(train_loader))
    batch_size = images.shape[0]
    stop_if(batch_size != BATCH_SIZE, f"preflight batch size {batch_size} != {BATCH_SIZE}")
    stop_if(tuple(images.shape[1:]) != (3, 224, 224), f"preflight image shape {tuple(images.shape)}")
    stop_if(labels.shape != (batch_size,), f"preflight label shape {tuple(labels.shape)}")

    images = images.to(device)
    labels = labels.to(device)
    preflight_model.eval()
    preflight_status = "PASS"
    preflight_loss_value = float("nan")
    try:
        with torch.no_grad():
            logits = preflight_model(images)
            stop_if(logits.ndim != 1, f"preflight logit ndim {logits.ndim} != 1")
            stop_if(logits.shape[0] != batch_size, f"preflight logit batch {logits.shape[0]}")
            stop_if(logits.shape != labels.shape, f"preflight {logits.shape} vs {labels.shape}")
            loss = criterion(logits, labels)
            preflight_loss_value = float(loss.item())
        print(f"Input shape: {tuple(images.shape)}")
        print(f"Label shape: {tuple(labels.shape)}")
        print(f"Logit shape: {tuple(logits.shape)}")
        print(f"BCEWithLogitsLoss (one batch): {preflight_loss_value:.6f}")
    except SystemExit:
        preflight_status = "FAIL"
        raise
    except Exception as exc:
        preflight_status = "FAIL"
        stop_if(True, f"preflight failed: {exc}")

    print(f"Preflight status: {preflight_status}")
    stop_if(preflight_status != "PASS", "Preflight FAILED — training aborted")
    ...
    model = EfficientNetB0BinaryV1(weights=DEFAULT_WEIGHTS)
    ...
    report_lines = [
        "EfficientNet-B0 Transfer Training Report V1",
        "==========================================",
        "",
        "Preflight",
        "---------",
        ...
        f"pretrained weight enum: {WEIGHTS_ENUM}",
        f"weights source: {DEFAULT_WEIGHTS.url}",
        f"device: {device}",
        f"total binary-model parameters: {preflight_total}",
        f"replacement binary head parameters: {preflight_head}",
        f"original classifier output size: {preflight_model.original_classifier_output_size}",
        f"replacement classifier output size: 1",
        f"preflight input shape: ({BATCH_SIZE}, 3, 224, 224)",
        f"preflight logit shape: ({BATCH_SIZE},)",
        f"preflight BCEWithLogitsLoss: {preflight_loss_value:.8f}",
        "",
        "Experiment",
        "----------",
        ...
        "Model",
        "-----",
        "architecture: EfficientNet-B0 (binary logit head)",
        ...
        "MobileNetV3-Small modified: NO",
        "EfficientNet training extended beyond predefined budget: NO",
        "",
    ...
    print("\n" + "=" * 60)
    print("STAGE 19 — EFFICIENTNET-B0 DEVELOPMENT COMPLETE")
    print("=" * 60)
    print("PREFLIGHT")
    print(f"Status: {preflight_status}")
    print(f"Device: {device}")
    print(f"Weights: {WEIGHTS_ENUM}")
    print(f"Parameters: {preflight_total}")
    print()
    ...
    print("Checkpoint:")
    print("models/efficientnet_b0_selected_v1.pt")
    print()
