"""Validation-only threshold selection for frozen MobileNetV3-Small (Stage 18C).

Why this file exists
--------------------
The selected Phase-2 MobileNet checkpoint is frozen. This script runs
...
What to expect
--------------
    results/mobilenet_v3_small_validation_predictions_v1.csv
    results/mobilenet_v3_small_threshold_selection_v1.txt
    results/mobilenet_v3_small_frozen_config_v1.json
"""
...
# Stage 18B reference values for reproduction check
STAGE18B_REF_AUC = 0.952331
STAGE18B_REF_AP = 0.956251
AUC_TOLERANCE = 0.005
AP_TOLERANCE = 0.005
...
PREDICTIONS_PATH = PROJECT_ROOT / "results" / "mobilenet_v3_small_validation_predictions_v1.csv"
REPORT_PATH = PROJECT_ROOT / "results" / "mobilenet_v3_small_threshold_selection_v1.txt"
...
def write_report(
    ...
    lines = [
        "MobileNetV3-Small Threshold Selection V1 — Stage 18C",
        "===================================================",
        "",
        "MODEL",
...
def main() -> None:
    ...
    stop_if(
        abs(roc_auc - STAGE18B_REF_AUC) > AUC_TOLERANCE,
        f"ROC-AUC {roc_auc:.6f} differs materially from Stage 18B reference {STAGE18B_REF_AUC}",
    )
    stop_if(
        abs(average_precision - STAGE18B_REF_AP) > AP_TOLERANCE,
        f"AP {average_precision:.6f} differs materially from Stage 18B reference {STAGE18B_REF_AP}",
    )
    ...
    print("STAGE 18C — MOBILENETV3-SMALL THRESHOLD SELECTION COMPLETE")
